...
- Non-PR workflow runs publish multi-arch `linux/amd64,linux/arm64` panel/API images to `ghcr.io/<owner>/neutrino-panel`.
- If `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN` are configured, the workflow also publishes to Docker Hub. `DOCKERHUB_IMAGE` repo variable overrides the Docker Hub image name; otherwise it defaults to `<DOCKERHUB_USERNAME>/neutrino-panel`.
