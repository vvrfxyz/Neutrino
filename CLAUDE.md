...
- `cmd/neutrinoctl`: node-side ops CLI (shipped in the agent image at `/usr/local/bin/neutrinoctl`): `status|queue|cert|enroll-info|test-xray|apply-preview|backups|rollback`, all over `internal/agent`'s `Ctl*` helpers (`ctl.go`) — read-only local state plus the existing fixed test/rollback paths; reads the same env as the agent; no shell proxy.
...
`internal/app`:
- `routes.go` — Go 1.22+ method+wildcard ServeMux patterns (`POST /api/v1/nodes/{id}/enable`); handlers read path params via `r.PathValue`, never by hand-parsing `r.URL.Path`. SSR pages: `/login`, `/users` (+ `/users/table` HTMX partial), `/users/{id}`, `/nodes`, `/nodes/{id}/deploy`, `/traffic`, `/enforcements`, `/apikeys`, `/ops`, `/ops-v2` (flag-gated). Public: `/sub/{token}`, `/healthz`. API: `/api/v1/*` including `ops/config|nodes|alerts`, backups, apikeys, and the admin WebSocket `/api/v1/stream` (session-only, same-origin checked). A `/api/` catch-all preserves auth-then-404 for unknown API paths.
- API error contract: every `/api/v1` JSON error goes through `apiError` (`{"error": message}`, see `http_helpers.go`). SSR pages and the public `/sub` endpoint keep plain-text errors; the per-event `results[].error` strings inside `POST /api/v1/usage` are a separate agent wire contract (do not reword).
...
`internal/agent` (`Agent.Run`): localhost health server, cert-renewer state machine (12h check, renew within 7d of expiry, atomic install + PanelClient hot-swap, re-enroll fallback when expired), Xray runtime guard (retries the synced-user restore until success, then probes Xray process uptime every 15s via `GetSysStats` and re-adds the committed baseline when an uptime regression reveals a restart), job runner (25s long-poll claim), usage loop (1s tick), heartbeat (2s report incl. online-IP snapshot, monthly RX/TX keyed by `AGENT_MONTH_TZ` → `time.Local` → UTC). The kernel boundary (`kernel.go`, module 6) names the seam to the local proxy core: `UserApplier` / `TrafficSampler` / `UptimeProber` (optional, type-asserted) / `RuntimeClient` / `ConfigApplier` — `internal/xrayapi.Client` and the xray file pipeline are today's only implementations; job dispatch goes through `a.configApplier()`. Behavior is unchanged; sing-box would implement the same surface.
...
Managed Xray: panel stores/ships only `{template, vars, rollback_on_fail, custom_outbounds, custom_routes}` (templates in `internal/templates/xray`; the custom outbound/route subset is the shared `internal/xraycfg` contract — allowlisted protocols freedom/blackhole/socks/http and domain/ip/port/network/protocol matchers, no `ext:`/`regexp:` rules). The agent renders with precedence job vars > non-placeholder env > agent-local fallbacks (REALITY keypair auto-generated and persisted in `reality.json`), re-validates and merges customs into the rendered config (custom rules insert right after the api rule), skips no-op applies via canonical-JSON compare, then backup → test → rename → reload using agent-local argv only (`XRAY_TEST_ARGS_JSON` / `XRAY_RELOAD_ARGS_JSON`). When customs are present the template carries `xraycfg.Marker`, so pre-module-4 agents fail the apply loudly instead of silently dropping the customs. `POST /api/v1/nodes/{id}/managed/xray/preview` renders the deploy preview (node-local vars stay as `${VAR}` placeholders). Never `sh -c`, never panel-supplied commands or file paths.

Subscription rendering is centralized in `internal/subscription`: `/sub/{token}` renders per-node URIs (vless_reality / hysteria2 / tuic) for `clash|singbox|v2rayn|shadowrocket` (UA auto-detect; `?target=` override with aliases `mihomo`/`clashmeta`/`stash`/`sing-box`, Xboard-style `?flag=` accepted; `?types=` protocol filter with `vless`/`hy2` alias spellings; `?filter=` case-insensitive `|`-OR node-name keywords — a filter matching nothing is a 400, never the stale active-link fallback; `?preset=default|global|minimal` picks the built-in clash profile shape — unknown presets 400, URI-list targets ignore it), applies each renderer's protocol capability set, emits `Subscription-Userinfo` / `Profile-Update-Interval` headers; falls back to env-config proxy only for unrestricted, unfiltered users when no nodes are enabled.
